...
async function loadProducts(){
    const res  = await fetch(api("products_api.php"), { ...withAuth });
...
async function deleteProduct(id){
    ...
    data.append("id", id);
    const res    = await fetch(api("products_api.php"), { method:"POST", body:data, ...withAuth });
...
document.getElementById("productForm").addEventListener("submit", async function(e){
    ...
    // Save product
    const res    = await fetch(api("products_api.php"), { method:"POST", body:formData, ...withAuth });
    const result = await res.json();
    ...
    for (const color of selectedColors){
        for (const size of selectedSizes){
            ...
            await fetch(api("products_api.php"), { method:"POST", body:vd, ...withAuth });
        }
    }
